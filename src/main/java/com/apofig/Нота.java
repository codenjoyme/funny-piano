...
public class Нота implements Comparable<Нота>{
    ...
    public static final List<String> НОТЫ = Arrays.asList(
    ...
    public static Нота parse(String string) {
        if (string.contains("(") && string.contains(")")) {
            String substring = string.substring(string.indexOf('(') + 1, string.indexOf(')'));
            return new Нота(Integer.valueOf(substring));
        } else {
            return new Нота(НОТЫ.indexOf(string));
        }
    }
}
